import { FaClipboardList, FaUsers, FaFlag, FaRocket, FaLightbulb } from "react-icons/fa";
    ...
    <div className="bg-gradient-to-r from-black via-[#0c0c0d] to-[#24033a] text-white py-20 px-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-10 max-w-7xl mx-auto mb-20">
        {steps.map((step, index) => (
          <div key={index} className="flex flex-col items-center text-center">
            <div className="bg-[#6B21A8]/20 p-6 rounded-xl mb-4">{step.icon}</div>
            <h3 className="text-xl font-medium mb-2 leading-tight">
              {step.title.split(" ").slice(0, 2).join(" ")}<br />{step.title.split(" ").slice(2).join(" ")}
            </h3>
            <p className="text-sm text-gray-300 max-w-[200px]">{step.description}</p>
          </div>
        ))}
      </div>
      ...
      <div className="max-w-6xl mx-auto divide-y divide-gray-600">
        {challenges.map((challenge, idx) => (
          ...
          >
            <span className="text-white text-lg font-mono w-12">[{challenge.number}]</span>
            <img src={challenge.image} alt={"challenge-" + challenge.number} className="w-44 h-24 object-cover rounded" />
            <p className="text-sm text-gray-200 flex-1 px-2">{challenge.text}</p>
